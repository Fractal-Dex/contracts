...
contract Minter is IMinter {
    ...
    uint public weekly = 1_838_000 * 1e18; // represents a starting weekly emission of 1.838M Token (Token has 18 decimals)
    ...
    function calculate_growth(uint _minted) public view returns (uint) {
        ...
        return
            (((((_minted * _veTotal) / _tokenTotal) * _veTotal) / _tokenTotal) *
                _veTotal) /
            _tokenTotal /
            2;
    }
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && initializer == address(0)) { // only trigger if new week
            ...
            require(_token.transfer(address(_rewards_distributor), _growth));
